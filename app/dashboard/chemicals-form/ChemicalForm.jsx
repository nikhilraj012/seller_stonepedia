  ...
  const uploadFiles = async (files, path) => {
    ...
    return Promise.all(
      fileArray.map(async (f) => {
        ...
        const fileRef = ref(storage, `chemicals/${uid}/${path}/${name}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!hasApprovedForm) {
        ...
        const ref1 = await addDoc(collection(db, "chemicals"), galleryData);
        await setDoc(
          doc(db, "SellerDetails", uid, "chemicals", ref1.id),
          galleryData,
        );
        console.log("Saved in SellerDetails also:", ref1.id);
        console.log("hu", productList);
        toast.success("Form submitted successfully!");
        router.push("/dashboard/profile/my-chemicals");
        setProductList([]);
      ...
      const gRef = doc(db, "chemicals", galleryId);
      const uRef = doc(db, "SellerDetails", uid, "chemicals", galleryId);
        ...
        {!hasApprovedForm ? (
          <>
            ...
            <p className="text-[#6E6E6E] text-sm md:text-md lg:text-lg xl:text-xl">
              Fill this form to Upload your Chemicals
            </p>
          </>
        ) : (
