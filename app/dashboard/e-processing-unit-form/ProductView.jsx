...
const ProductView = ({
  ...
  const handleImageDelete = (index, imgIndex) => {
    setProductList((prevList) =>
      prevList.map((product, i) => {
        if (i === index) {
          const removed = product.media?.[imgIndex];
          if (removed?.url) {
            try {
          ...
          return {
            ...product,
            media: (product.media || []).filter((_, ii) => ii !== imgIndex),
          };
        }
        return product;
      }),
        ...
        {productList.map((p, i) => (
          ...
          >
            <div className={`flex justify-between`}>
              <div className="flex gap-3">
                ...
                <div className="text-xs md:text-sm space-y-0.5">
                  ...
                  <p className="text-[#8B8B8B]">{p.stoneCategory}</p>
                </div>
                <div></div>
              </div>
            ...
            <div className="border p-2 rounded-md space-y-1 border-gray-300">
              <div className="text-[10px] md:text-xs text-[#636363] space-y-1">
                <div className="flex">
                  <span className="font-medium text-black w-1/3">
                    Available Finish:
                  </span>
                  <span className="w-2/3">{displayArray(p.finish)}</span>
                </div>
                ...
                <div className="flex">
                  ...
                  <span className="w-2/3">{displayArray(p.thickness)}</span>
                </div>
                ...
                <div className="flex">
                  ...
                  <span className="w-2/3 flex gap-1">
                    <p>{p.units?.label || p.units}</p>
                  </span>
                </div>
                ...
                <div className="flex">
                  ...
                  <span className="w-2/3">
                    H -{" "}
                    {p.size.height?.length
                      ? p.size.height.map((h) => `(${h})`).join(", ")
                      : "-"}{" "}
                    | W -{" "}
                    {p.size.width?.length
                      ? p.size.width.map((w) => `(${w})`).join(", ")
                      : "-"}
                  </span>
                </div>
              </div>
