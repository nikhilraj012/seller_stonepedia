...
const page = () => {
  ...
  const handleViewDetails = (item, product) => {
    ...
    router.push(
      `/dashboard/profile/my-chemicals/${companySlug}/${productSlug}`,
    );
  };
  const openEdit = (item) => {
    setEditId(item.id);
    ...
    return (
      <div className="flex flex-col justify-center items-center min-h-screen text-center">
        ...
        <p className="text-gray-500 mb-4">
          You haven’t added anything to Chemicals-form yet.
        </p>

        <button
          onClick={() => router.push("/dashboard/chemicals-form")}
  ...
  return (
    <div className="pt-16 px-4 md:px-6 lg:px-24 xl:px-32">
      ...
      {chemicals.every((item) => item.status === "rejected") && (
          ...
          <div className="space-y-4 mt-3">
            {paginatedData.map((item, i) => (
              <GalleryCard
                ...
                item={item}
                updating={updating}
                addSlabRoute="/dashboard/chemicals-form"
                actions={{
                  handleCancel,
                  updateThumbnail,
                  onEdit: openEdit,
