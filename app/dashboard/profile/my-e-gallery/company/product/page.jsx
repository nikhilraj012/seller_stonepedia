...
const [loading, setLoading] = useState(true);
  ...
  const MODE_CONFIG = {
    ...
    processing: {
      collection: "EGalleryForProcessingUnit",
      basePath: "/e-processing-unit",
    },
  };
  const config = MODE_CONFIG[mode];
  const { collection, basePath } = config;
...
useEffect(() => {
  ...
  const fetchData = async () => {
    ...
    const qSnap = await getDocs(
      fbCollection(db, "SellerDetails", uid, config.collection),
    );
    ...
    qSnap.forEach((docSnap) => {
      ...
      if (toSlug(data.companyDetails.shopName) === companySlug) {
        const prod = data.products.find(
          (p) => toSlug(p.stockDetails.stoneName) === productSlug,
        );
  ...
  const handleThumbnailChange = async (e) => {
    ...
    try {
      ...
      console.log(item.id);
      const gRef = doc(db, collection, item.id);
      const uRef = doc(db, "SellerDetails", user.uid, collection, item.id);
  ...
  const handleThumbnailRemove = async (productId) => {
    ...
    try {
      ...
      const userRef = doc(db, "SellerDetails", user.uid, collection, item.id);
      const globalRef = doc(db, collection, item.id);
